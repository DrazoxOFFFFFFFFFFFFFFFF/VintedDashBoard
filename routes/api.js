...
router.put('/items/:id', async (req, res) => {
  try {
    ...
    const item = await get('SELECT * FROM items WHERE id = ? AND user_id = ?', [req.params.id, req.userId]);
...
router.delete('/items/:id', async (req, res) => {
  try {
    ...
    await run('DELETE FROM items WHERE id = ? AND user_id = ?', [req.params.id, req.userId]);
    await run('DELETE FROM transactions WHERE itemId = ? AND user_id = ?', [req.params.id, req.userId]);
...
router.get('/transactions', async (req, res) => {
  try {
    await getDb();
    const txns = await query('SELECT * FROM transactions WHERE user_id = ? ORDER BY date DESC', [req.userId]);
    res.json(txns);
  } catch (err) { res.status(500).json({ error: 'Erreur serveur' }) }
});
...
router.delete('/transactions/:id', async (req, res) => {
  try {
    await getDb();
    const txn = await get('SELECT * FROM transactions WHERE id = ? AND user_id = ?', [req.params.id, req.userId]);
...
router.get('/settings', async (req, res) => {
  try {
    await getDb();
    const user = await get('SELECT goal, currency FROM users WHERE id = ?', [req.userId]);
...
router.put('/settings', async (req, res) => {
  try {
    ...
    await run('UPDATE users SET goal = ?, currency = ? WHERE id = ?', [goal ?? 2000, currency || '€', req.userId]);
